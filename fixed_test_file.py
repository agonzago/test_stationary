...
def run_stationary_prior_transformation_test(
    ...
    with pm.Model() as test_model:
        ...
        gamma_list_transformed_pt = [] # Default to empty list
        if p_order > 0:
            # make_stationary_var_transformation expects Sigma (error cov) and A_list (untransformed params)
            # It returns phi_list, gamma_list
            phi_list_transformed_pt, gamma_list_transformed_pt = make_stationary_var_transformation(
                Sigma_input_pt,
                A_list_pt_input,
                m_dim,
                p_order
            ...
            # Also store the gamma (autocovariance) matrices
            for i_gamma in range(p_order):
                pm.Deterministic(f"gamma_transformed_{i_gamma}", gamma_list_transformed_pt[i_gamma])
            pm.Deterministic("gamma_stacked_transformed", pt.stack(gamma_list_transformed_pt, axis=0))
            ...
            pm.Deterministic("gamma_stacked_transformed",
                             pt.zeros(empty_phi_shape, dtype=pytensor.config.floatX))
...
if __name__ == '__main__':
    ...
    for i, case_params in enumerate(test_cases):
        ...
        # Fixed: Calculate total_checked for each result
        total_checked = res["stationary_samples_count"] + res["non_stationary_samples_count"]
        
        if res["stationarity_ratio_of_checked"] < expected_min_ratio:
            if total_checked > 0: # Avoid division by zero if all checks failed
            ...
            if total_checked > 0: # Only print pass if samples were checked
