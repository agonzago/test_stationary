...
def run_stationary_prior_jax_test(
    ...
    results = {
        ...
        "test_passed": True  # ADDED: Track if this test case passed
    }
    ...
    results["fail_messages"] = fail_messages
...
if __name__ == "__main__":
    ...
    for i, case_params in enumerate(test_cases):
        ...
        # FIXED: Check the actual test_passed field from the results
    ...
    # FIXED: Use the corrected logic to determine overall pass/fail
    if all_test_cases_passed:
        ...
        # Print details about which tests failed
        failed_cases = [i+1 for i, res in enumerate(all_results) if not res["test_passed"]]
        if failed_cases:
            print(f"Failed test cases: {failed_cases}")
            for i, res in enumerate(all_results):
                if not res["test_passed"]:
                    print(f"\nCase {i+1} (m={res['m']}, p={res['p']}) failed:")
                    for msg in res.get("fail_messages", []):
                        print(f"  - {msg}")